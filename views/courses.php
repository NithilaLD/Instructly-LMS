<?php
    ...
    function downloadCoursesTemplate()
    {
        ...
        $stmt = $mysqli->prepare("SELECT name, user_code FROM users WHERE role = 'instructor' AND status = 'active'");
        $stmt->execute();
    ...
    if (isset($_POST['add_course_cat']) && ($role === 'admin' || $role === 'instructor'))
    {
        ...
        if (!$error) {
            ...
            $stmt = $mysqli->prepare("SELECT * FROM courses WHERE c_code = ? AND status <> 'deleted'");
            ...
            if (mysqli_num_rows($res) > 0) {
                ...
                if ($stmt) {
                    ...
                    logAuditAction($mysqli, 'add_course', 'Course created', 'courses', 'course', $c_code);
    ...
    if (isset($_POST['update_course_cat']) && ($role === 'admin' || $role === 'instructor'))
    {
        ...
        if (!$error) {
            ...
            $stmt->execute();
            if ($stmt) {
                logAuditAction($mysqli, 'update_course', 'Course updated', 'courses', 'course', (string) $c_id);
    ...
    if (isset($_GET['delete']) && $role === 'admin')
    {
        ...
        $adn = "UPDATE courses SET status = 'deleted' WHERE c_id = ?";
        $stmt = $mysqli->prepare($adn);
        $stmt->bind_param('i', $id);

        if ($stmt->execute()) {
            logAuditAction($mysqli, 'delete_course', 'Course deleted', 'courses', 'course', (string) $id);
                                                            ...
                                                            <div class="col-md-6">
                                                                ...
                                                                <div class="form-group">
                                                                    ...
                                                                    <select name="c_ins" class="form-control select2bs4" style="width: 100%;" id="c_ins">
                                                                        <?php
                                                                        $ret = "SELECT * FROM users WHERE role = 'instructor' AND status = 'active'";
                                                                        $stmt = $mysqli->prepare($ret);
                                                                        $stmt->execute(); //ok
                                                                        $res = $stmt->get_result();
                                    ...
                                    <div class="card-body">
                                        <table id="dash-2" class="table table-striped table-bordered display " style="width:100%">
                                            ...
                                            <?php
                                                if ($role === 'admin')
                                                {
                                                    $ret = "SELECT * FROM courses WHERE status <> 'deleted'";
                                                    $stmt = $mysqli->prepare($ret);
                                                }
                                                else if($role === 'instructor')
                                                {
                                                    $ret = "
                                                        ...
                                                        WHERE u.role = 'instructor' AND u.status = 'active' AND c.status <> 'deleted' AND c.i_id = ?
                                                    ";
                                                    $stmt = $mysqli->prepare($ret);
                                                ...
                                                else if ($role === 'student')
                                                {
                                                    $ret = "
                                                        ...
                                                        WHERE e.s_id = ? AND e.status <> 'deleted' AND c.status <> 'deleted'
                                                ...
                                                while ($courses = $res->fetch_object())
                                                {
                                                    ...
                                                    {
                                                        $inc= "SELECT * FROM users WHERE user_id = '$courses->i_id' AND status = 'active'";
