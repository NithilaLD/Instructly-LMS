<?php
    ...
    function fetchQuestionsForRole(mysqli $mysqli, string $role, int $user_id): array
    {
        if ($role === 'admin') {
            ...
            $sql = "
                ...
                LEFT JOIN courses c ON u.c_id = c.c_id
                LEFT JOIN users usr ON q.i_id = usr.user_id
                WHERE e.s_id = ? AND e.status <> 'deleted' AND q.status <> 'deleted'
                ORDER BY q.q_id DESC
            ";
            $stmt = $mysqli->prepare($sql);
            $stmt->bind_param('i', $user_id);
    ...
    {
        $stmt = $mysqli->prepare("SELECT an_id, q_id, an_code, ans_details FROM answers WHERE q_id = ? AND status <> 'deleted' LIMIT 1");
    ...
    if (isset($_POST['add_answer']) && ($role === 'admin' || $role === 'instructor')) {
        ...
        if ($q_id <= 0) {
            ...
            $stmt = $mysqli->prepare("SELECT q_id FROM questions WHERE q_id = ? AND status <> 'deleted' LIMIT 1");
            ...
            if (!$qRow) {
                ...
                $stmt = $mysqli->prepare("SELECT an_id FROM answers WHERE q_id = ? AND status <> 'deleted' LIMIT 1");
                $q_id_str = (string) $q_id;
                $stmt->bind_param('s', $q_id_str);
                $stmt->execute();
                ...
                if ($exists) {
                    ...
                    $stmt = $mysqli->prepare("
                        INSERT INTO answers (q_id, an_code, ans_details, status)
                        VALUES (?, ?, ?, 'added')
                    ");
                    $q_id_str = (string) $q_id;
                    ...
                    if ($stmt->execute()) {
                        logAuditAction($mysqli, 'add_answer', 'Answer added', 'answers', 'answer', (string) $q_id);
    ...
    if (isset($_POST['update_answer']) && ($role === 'admin' || $role === 'instructor')) {
        ...
        if (!$error) {
            $query = "UPDATE answers SET ans_details = ?, an_code = ?, status = 'updated' WHERE an_id = ?";
            $stmt = $mysqli->prepare($query);
            $stmt->bind_param('ssi', $ans_details, $an_code, $an_id);

            if ($stmt->execute()) {
                logAuditAction($mysqli, 'update_answer', 'Answer updated', 'answers', 'answer', (string) $an_id);
    ...
    if ($role === 'admin' && isset($_GET['delete'])) {
        ...
        $stmt = $mysqli->prepare("UPDATE answers SET status = 'deleted' WHERE an_id = ?");
        $stmt->bind_param('i', $id);

        if ($stmt->execute()) {
            logAuditAction($mysqli, 'delete_answer', 'Answer deleted', 'answers', 'answer', (string) $id);
