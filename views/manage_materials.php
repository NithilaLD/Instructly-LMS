<?php
    ...
    if ($u_id === '')
    {
        header("Location: unit_materials.php");
        exit();
    }
    ...
    if (isset($_GET['delete']) && ($role == 'instructor' || $role == 'admin'))
    {
        ...
        // get file name first
        $fileStmt = $mysqli->prepare("SELECT m_name FROM materials WHERE m_id = ?");
        $fileStmt->bind_param("i", $id);
        $fileStmt->execute();
        $fileRes = $fileStmt->get_result();

        if ($fileRow = $fileRes->fetch_object())
        {
            $filePath = "../public/sys_data/uploads/materials/" . $fileRow->m_name;
        ...
        // delete DB record
        $adn = "DELETE FROM materials WHERE m_id = ?";
        ...
        if ($stmt) {
            ...
            header("Location: manage_unit_materials.php?u_id=" . urlencode($u_id));
            exit;
        } else {
                                                                ...
                                                                <div class="modal-dialog modal-dialog-centered" role="document">
                                                                    <div class="modal-content">
                                                                        ...
                                                                        <div class="modal-body text-center text-danger">
                                                                            ...
                                                                            <a href="manage_unit_materials.php?u_id=<?php echo $u_id; ?>&delete=<?php echo $materials->m_id; ?>" class="text-center btn btn-outline-warning">Delete</a>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </div>
