    ...
    if (isset($_POST['update_profile']))
    {
        ...
        { 
            ...
            else
            {
                ...
                if ($stmt->execute())
                {
                    logAuditAction($mysqli, 'update_profile', 'Profile details updated', 'profile', 'user', (string) $user_id);
        ...
        else
        {
            ...
            if ($row = $res->fetch_assoc())
            {
                if ($row['password'] === $hashed_new) { $_SESSION['flash_error'] = "New password and the old password cannot be the same"; }
                    ...
                    if ($stmt->execute()) {
                        logAuditAction($mysqli, 'update_password', 'Password changed from profile page', 'profile', 'user', (string) $user_id);
