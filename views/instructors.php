<?php
        ...
        if (!$error) {
            ...
            if (mysqli_num_rows($res) > 0) {
                ...
                if ($stmt) {
                    ...
                    logAuditAction($mysqli, 'add_instructor', 'Instructor created', 'instructors', 'user', (string) $insertId);
    ...
    if (isset($_POST['update_ins'])) {
        ...
        if (!$error) {
            ...
            $query = "UPDATE users SET user_code =?, name =?, phone =?, email =?, dpic =?, status = 'updated' WHERE user_id = ?";
            $stmt = $mysqli->prepare($query);
            $rc = $stmt->bind_param('ssssss', $i_number, $i_name, $i_phone, $i_email, $newImageName, $i_id);
            $stmt->execute();
            if ($stmt) {
                logAuditAction($mysqli, 'update_instructor', 'Instructor updated', 'instructors', 'user', (string) $i_id);
    ...
    if (isset($_GET['delete'])) {
        ...
        if ($stmt->execute()) {
            logAuditAction($mysqli, 'delete_instructor', 'Instructor deleted', 'instructors', 'user', (string) $id);
    ...
    if (isset($_POST['add_teaching_allocation'])) {
        ...
        if (!$error && isset($i_id))
        {
            ...
            else
            {
                ...
                logAuditAction($mysqli, 'add_teaching_allocation', 'Teaching allocation added', 'instructors', 'course', (string) $c_id);
    ...
    if (isset($_POST['remove_teaching_allocation'])) {
        ...
        if ($stmt2->execute())
        {
            logAuditAction($mysqli, 'remove_teaching_allocation', 'Teaching allocation removed', 'instructors', 'course', (string) $c_id);
                                ...
                                <div class="col-md-12">
                                    ...
                                    <div class="card-body">
                                        <table id="dash-1" class="table table-striped table-bordered display no-wrap" style="width:100%">
                                            ...
                                            <tbody>
                                                <?php
                                                $uret = "SELECT  * FROM  users WHERE role = 'instructor' AND status = ('active' || 'updated') ";
                                                $ustmt = $mysqli->prepare($uret);
                                                $ustmt->execute(); //ok
                                                $ures = $ustmt->get_result();
                                                while ($ins = $ures->fetch_object()) {
                                                ?>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>
                                                                                ...
                                                                                <div class="row">
                                                                                    <div class="form-group col-md-6">
                                                                                        ...
                                                                                        <select name="c_id" style="width: 100%;" id="Course" required class="form-control select2bs4">
                                                                                            ...
                                                                                            <?php
                                                                                                $cstmt = $mysqli->prepare("SELECT * FROM courses WHERE (i_id != ? OR i_id IS NULL) AND status <> 'deleted'");
                                                                                                $cstmt->bind_param("s", $ins->user_id);
                                                                                                $cstmt->execute();
                                                                                                $cres = $cstmt->get_result();
                                                                                                while ($course = $cres->fetch_object())
                                                                                                {
                                                                                            ?>
                                                            ...
                                                            <!-- End Delete Modal -->
                                                        <?php
                                                                $stmt = $mysqli->prepare("SELECT * FROM courses WHERE i_id = ? AND status <> 'deleted'");
