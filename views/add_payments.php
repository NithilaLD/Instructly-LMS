...
if (isset($_POST['pay_for_reading_material'])) {
    ...
    if (!$error) {
        ...
        if (mysqli_num_rows($res) > 0) {
            ...
            $stmt->execute();
            $query1 = "UPDATE lms_study_material SET payment_status = ? WHERE ls_id = ?";
            $stmt1 = $mysqli->prepare($query1);
            $stmt1->bind_param("si", $payment_status, $ls_id);
            $stmt1->execute();
            if ($stmt && $stmt1) {
                    ...
                    <section class="content">
                        <div class="container">
                            <div class="text-right text-dark">
                                <a class="btn btn-outline-warning" href="std_manage_payments.php">Manage My Payments</a>
                            </div>
                        </div>
                        <hr>
                                    ...
                                    <div class="card-body">
                                        <table id="dash-1" class="table table-striped table-bordered display no-wrap" style="width:100%">
                                            ...
                                            <tbody>
                                                <?php
                                                $id = $_GET['view'];
                                                $ret = "SELECT  *  FROM  lms_study_material WHERE c_id = '$id' && payment_status = 'unpaid' ";
                                                $stmt = $mysqli->prepare($ret);
                                                $stmt->execute(); //ok
                                                $res = $stmt->get_result();
                                                                            ...
                                                                            <form method="post" enctype="multipart/form-data">
                                                                                ...
                                                                                <div class="row">
                                                                                    ...
                                                                                    <div class="form-group col-md-12">
                                                                                        <label for="exampleInputEmail1">Payment Code | Refrence Number <small class="text-warning">Credit/Debit Card - Transaction ID | Online Transfer - Bank Reference No. | Bank Deposit - Slip No.</small></label>
