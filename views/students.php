<?php
    ...
    function downloadStudentTemplate()
    {
        ...
        $stmt = $mysqli->prepare("SELECT c_name FROM courses WHERE status <> 'deleted'");
        $stmt->execute();
    ...
    if (isset($_POST['enroll_student']) && ($role === 'admin' || $role === 'instructor')) {
        ...
        if (!$error) {
            $stmt = $mysqli->prepare("SELECT * FROM enrollments WHERE s_id = ? AND c_id = ? AND status <> 'inactive'");
            ...
            if (mysqli_num_rows($res) > 0) {
                ...
                if ($stmt) {
                    logAuditAction($mysqli, 'enroll_student', 'Student enrolled in a course', 'students', 'enrollment', $c_id . ':' . $s_id);
    ...
    if (isset($_POST['add_std']) && ($role === 'admin' || $role === 'instructor')) {
        ...
        if (!$error) {
            $stmt = $mysqli->prepare("SELECT * FROM users WHERE user_code = ? AND status = 'active'");
            ...
            if (mysqli_num_rows($res) > 0) {
                ...
                if ($stmt) {
                    ...
                    logAuditAction($mysqli, 'add_student', 'Student created', 'students', 'user', (string) $studentInsertId);
    ...
    if (isset($_POST['update_student']) && ($role === 'admin' || $role === 'instructor')) {
        ...
        if (!$error) {
            ...
            $query = "UPDATE users SET user_code =?, name =?, email =?, phone =?, dpic =?, status = 'updated' WHERE user_id = ?";
            $stmt = $mysqli->prepare($query);
            $rc = $stmt->bind_param('sssssi', $s_regno, $s_name, $s_email, $s_phoneno, $s_dpic, $s_id);
            $stmt->execute();
            if ($stmt) {
                logAuditAction($mysqli, 'update_student', 'Student updated', 'students', 'user', (string) $s_id);
    ...
    if (isset($_GET['delete']) && $role === 'admin') {
        ...
        $adn = "UPDATE users SET status = 'deleted' WHERE user_id = ?";
        $stmt = $mysqli->prepare($adn);
        $stmt->bind_param('i', $id);

        if ($stmt->execute()) {
            logAuditAction($mysqli, 'delete_student', 'Student deleted', 'students', 'user', (string) $id);
    ...
    if (isset($_POST['unenroll_student']) && $role === 'admin') {
        ...
        if (!$error)
        {
            $stmt = $mysqli->prepare("UPDATE enrollments SET status = 'inactive' WHERE s_id = ? AND c_id = ?");
            $stmt->bind_param("ss", $s_id, $c_id);
            if ($stmt->execute()) {
                logAuditAction($mysqli, 'delete_enrollment', 'Enrollment deleted', 'students', 'enrollment', $c_id . ':' . $s_id);
                $_SESSION['flash_success'] = "Enrollment deleted successfully.";
                header("Location: students.php");
            } else {
                $_SESSION['flash_error'] = "Failed to delete enrollment.";
                header("Location: students.php");
            }
        }
    }
                                                            ...
                                                            <div class="form-group col-md-6">
                                                                ...
                                                                <select class="custom-select" id="inputGroupSelect03" name="s_course">
                                                                    ...
                                                                    <?php
                                                                        ...
                                                                        if ($role === 'admin')
                                                                        {
                                                                            $aret = "SELECT * FROM courses WHERE status <> 'deleted'";
                                        ...
                                        <table id="dash-1" class="table table-striped table-bordered display no-wrap" style="width:100%">
                                            ...
                                            <tbody>
                                                <?php
                                                    $vstmt = null;
                                                    $onrole = "student";
                                                    $vret = "SELECT * FROM users WHERE role = ? AND status = ('active' || 'updated')";
                                                                                        ...
                                                                                        <div class="form-group col-md-6">
                                                                                            ...
                                                                                            <select name="enc_id" style="width: 100%;" id="Course" required class="form-control select2bs4">
                                                                                                ...
                                                                                                <?php
                                                                                                    ...
                                                                                                    if ($role === 'admin')
                                                                                                    {
                                                                                                        $cret = "SELECT c_id, c_code, c_name FROM courses WHERE status <> 'deleted'";
                                                                                                        $cstmt = $mysqli->prepare($cret);
                                                                                                    }
                                                                                                    else
                                                                                                    {
                                                                                                        $cret = "SELECT c_id, c_code, c_name FROM courses WHERE i_id = ? AND status <> 'deleted'";
                                                                                                        $cstmt = $mysqli->prepare($cret);
                                                                                                    ...
                                                                                                    {
                                                                                                        $eret = "SELECT * FROM enrollments WHERE s_id = ? AND c_id = ? AND status <> 'deleted'";
