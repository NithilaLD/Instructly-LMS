    ...
    ?>
        ...
        <body class="hold-transition sidebar-mini layout-fixed layout-navbar-fixed layout-footer-fixed">
            <div class="wrapper">
                ...
                $ret = "SELECT * FROM courses where c_id = ? AND status <> 'rejected'";
                $stmt = $mysqli->prepare($ret);
                ...
                while ($course = $res->fetch_object()) {
                    if($course->i_id != null)
                    {
                        $inc= "SELECT * FROM users WHERE user_id = '$course->i_id' AND status = 'active'";
                                                ...
                                                <div class="tab-content">
                                                    <div class="active tab-pane" id="units">
                                                        <table id="dash-1" class="table table-striped table-bordered display " style="width:100%">
                                                            ...
                                                            <tbody>
                                                                <?php
                                                                $stmt = $mysqli->prepare("SELECT * FROM units WHERE c_id = ? AND status <> 'rejected'");
                                                                $stmt->bind_param("i", $view);
                                                                $stmt->execute();
                                                                $res = $stmt->get_result();
                                                            ...
                                                            <tbody>
                                                                <?php
                                                                    $enrollstmt = $mysqli->prepare("
                                                                        ...
                                                                        WHERE e.c_id = ? AND e.status <> 'inactive' AND c.status <> 'rejected' AND ins.status = 'active' AND stu.status = 'active'
                                                                    ");
