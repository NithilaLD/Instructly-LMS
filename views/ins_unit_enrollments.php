...
if (isset($_POST['enroll_student'])) {
    ...
    if (!$error) {
        $sql = "SELECT * FROM  lms_enrollments WHERE  s_regno = ? AND s_unit_code = ?";
        $stmt = $mysqli->prepare($sql);
        $stmt->bind_param('ss', $s_regno, $s_unit_code);
        $stmt->execute();
        $res = $stmt->get_result();
        if ($res->num_rows > 0) {
            $row = $res->fetch_assoc();
            if (($s_regno == $row['s_regno']) && ($s_unit_code == $row['s_unit_code'])) {
                $unitName = (isset($s_unit_name) && !empty($s_unit_name)) ? $s_unit_name : (isset($row['s_unit_name']) ? $row['s_unit_name'] : 'this unit');
                $err =  $row['s_name'] . " Already Enrolled To " . $unitName;
            }
        } else {
                                                ...
                                                <form method="post" enctype="multipart/form-data">
                                                    <div class="row">
                                                        <div class="form-group col-md-6">
                                                            <label for="Allocated_Unit_Code">Unit Code</label>
                                                        ...
                                                        <div class="form-group col-md-6">
                                                            <label for="Allocated_Unit_Name">Unit Name</label>
                                                            <input type="text" id="Allocated_Unit_Name" name="s_unit_name" required class="form-control" autocomplete="name">
                                                    ...
                                                    <div class="row">
                                                        <div class="form-group col-md-6">
                                                            <label for="Std_Admn">Student Admission Number</label>
                                                        ...
                                                        <div class="form-group col-md-6">
                                                            ...
                                                            <input type="text" name="s_name" id="Std_Name" required class="form-control" autocomplete="name">
