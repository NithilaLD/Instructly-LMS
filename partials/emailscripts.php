<?php
    ...
    function addUniqueMediaItem(array &$bucket, mixed $value): void
    {
        ...
        if ($value === '' || in_array($value, $bucket, true)) { return; }
        $bucket[] = $value;
    }
    function ensureDirectoryExists(string $dirPath): void { if (!is_dir($dirPath)) { mkdir($dirPath, 0777, true); } }
    function cleanupExtractedMediaFiles(?array $mediaAssets): void
    {
        if (!is_array($mediaAssets) || empty($mediaAssets['attachments'])) { return; }
    ...
    function extractMatchingKeywords(string $text): array
    {
        ...
        if ($text === '') { return []; }
        $stopWords = array_fill_keys(['the', 'and', 'for', 'with', 'that', 'this', 'from', 'into', 'your', 'have', 'has', 'was', 'were', 'are', 'you', 'will', 'been', 'can', 'any', 'all', 'new', 'course', 'material', 'module', 'lms', 'open', 'now', 'about', 'part', 'lab', 'virtual', 'machine', 'workstation' ], true);
        ...
        {
            $token = trim($token);
            if ($token === '' || strlen($token) < 4 || isset($stopWords[$token])) { continue; }
            $keywords[$token] = true;
        }
    ...
    {
        if (!class_exists('thiagoalessio\\TesseractOCR\\TesseractOCR') || !is_file($filePath)) { return ''; }
        try
        {
    ...
    {
        if (!is_array($asset) || empty($asset['path']) || !is_file($asset['path'])) { return 0; }
        $keywords = extractMatchingKeywords($contextText);
        if (empty($keywords)) { return 1; }
        $haystacks = [
    ...
    function extractMediaMetadata(string $filePath, string $mimeType, ?array &$mediaAssets = null): array {
        ...
        if (!is_array($mediaAssets)) { $mediaAssets = createMediaAssetStore(); }
    ...
    function generateNotification(string $fileContent, array $media, array $mediaAssets, string $moduleName, string $materialTitle, string $apiKey): array {
